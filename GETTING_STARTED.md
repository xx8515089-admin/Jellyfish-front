...
Java 后端服务启动后（例如 `http://localhost:8086/jellyfish`），在前端目录执行：
...
默认请求基础地址来自 `front/.env` 的 `VITE_BACKEND_URL`（见 `src/services/openapi.ts`）。部署环境可通过 `public/env.js` 注入 `window.__ENV.BACKEND_URL` 覆盖构建期配置。
