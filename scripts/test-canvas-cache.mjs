...
function harness(api = {}) {
  ...
  const workspaces = load('../src/pages/canvas/canvasWorkspaces.ts', {'./TapnowStudio/canvasOperationLock':{withCanvasOperationLock},'./canvasCache':cache,'../../services/studioCanvases':{StudioCanvases:api,canvasRequestId:()=> 'create-test'}}, {window})
...
test('delete clears only target canvas caches, preserving recovery records and clearing disposable databases', async () => {
...
test('successful remote delete writes a tombstone and preserves pending cloud receipts', async () => {
  ...
  assert.equal(values.has('canvas-cloud:42:7:save'),true)
})
...
test('creating a pristine canvas clears obsolete local content but leaves other canvases intact', async () => {
  ...
  assert.equal(values.get('canvas-cloud:42:8:draft'), 'keep')
  assert.equal(values.get('canvas-cloud:42:7:draft'), 'obsolete')
  assert.equal(values.has('jellyfish_canvas:7:tapnow_autosave'), false)
})
