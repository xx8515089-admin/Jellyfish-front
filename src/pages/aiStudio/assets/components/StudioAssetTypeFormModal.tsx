...
export function StudioAssetTypeFormModal({
  ...
  const handleOk = async () => {
    if (!formName.trim()) {
      message.warning(l('请输入资产名称', 'Enter an asset name'))
  ...
  return (
    <Modal
      title={editing ? l(`编辑${label}`, `Edit ${label}`) : l(`新建${label}`, `New ${label}`)}
      open={open}
      onCancel={onCancel}
      onOk={handleOk}
      okText={l('保存', 'Save')}
    ...
    >
      <div className="space-y-3">
        ...
        <div>
          <span className="text-gray-600 text-sm">{l('标签（逗号分隔）', 'Tags (comma-separated)')}</span>
          <Input value={formTags} onChange={(e) => setFormTags(e.target.value)} className="mt-1" />
        </div>
        <div>
          <span className="text-gray-600 text-sm">{l('镜头数', 'View count')}</span>
          <InputNumber
            min={0}
            max={4}
            precision={0}
            ...
            placeholder={l('例如 4（最大 4）', 'For example: 4 (maximum 4)')}
          />
        </div>
        <div>
