...
export default function CanvasBatchPanel({ session, candidates, save, onClose, onBatch }) {
  ...
  return <Modal open title={uiText("批量生成")} onCancel={busy ? undefined : onClose} footer={null} width={650}>
    <p>{uiText("选择同一版本的独立任务。结果保留在历史中，确认应用后才改变画布。")}</p>
    ...
    <Space wrap><span>{uiText("每项数量")}</span><InputNumber min={1} max={10} precision={0} disabled={busy} value={count} onChange={value => { setCount(value || 1); invalidate() }} /><span>{uiText("共") + " "}{selected.length * count} {uiText("个任务（最多 50）")}</span></Space>
    {error && <Alert type="error" message={error} />}
    {quote && <Alert type="info" message={uiText("已预估版本 v{0} · {1}", quote.body.revisionNo, quote.total == null ? uiText("请填写允许预留的积分上限") : `预留积分 ${quote.total}`)} description={uiText("预留上限只限制提交时预留积分，不是供应商实际费用的最终结算封顶。")} />}
    {quote?.estimate.items?.length > 0 && <div style={{ maxHeight: 150, overflow: 'auto', marginTop: 10 }}>{quote.estimate.items.map((item, index) => <p key={item.clientItemId || index}>{candidates.find(candidate => candidate.clientItemId === item.clientItemId)?.label || uiText("任务 {0}", index + 1)}：{item.unitCredits ?? item.unitCreditCost ?? item.creditCost ?? '—'} {uiText("积分/项 · 数量") + " "}{item.count ?? count} {uiText("· 小计预留") + " "}{item.subtotalCredits ?? (item.unitCredits ?? item.estimate?.creditCost ?? 0) * (item.count ?? count)} {uiText("积分")}</p>)}</div>}
    {quote && <Space style={{ marginTop: 12 }}><span>{uiText("预留积分上限")}</span><InputNumber min={0} value={budget} disabled={busy} onChange={setBudget} /></Space>}
    <Space style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 20 }}><Button disabled={busy} onClick={onClose}>{uiText("关闭")}</Button><Button loading={busy} disabled={!selected.length || selected.length * count > 50} onClick={() => run(async () => {
    ...
    })}>{uiText("预估费用")}</Button><Button type="primary" disabled={busy || !quote || budget == null || budget < 0} onClick={() => run(async () => { onBatch(await session.submitBatch({ ...quote.body, maxTotalCredits: budget })); onClose() })}>{uiText("确认提交")}</Button></Space>
  </Modal>
}
...
function LegacyCanvasBatchHistory({ session, onTask, report, revision }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (stopped) return
        setBatches(results)
        if (results.some(batch => batch.shouldPoll)) timer = setTimeout(load, canvasPollDelay(...results))
      } catch (e) { if (!stopped) { report(e); if (![401, 403, 410].includes(e.status)) timer = setTimeout(load, canvasPollDelay(e)) } }
    }
    void load()
  ...
  return <div className="canvas-history-list"><Space><Button disabled={busy} onClick={() => setIds([...ids])}>{uiText("刷新批次")}</Button>{session.pendingBatch && <Button disabled={busy} onClick={() => run(async () => remember(await session.recoverBatch()))}>{uiText("找回批次")}</Button>}</Space>{batches.map(batch => <section className="canvas-history-card" key={batch.batchId}><strong>{uiText("批次") + " "}{batch.batchId}</strong> <Tag>{labels[batch.status] || batch.status}</Tag><p>{uiText("首次预留：")}{batch.reservedCredits ?? uiText("未知")} {uiText("积分 ·") + " "}{batch.items?.length || 0} {uiText("个任务")}</p><Space wrap>{batch.shouldPoll && <Button disabled={busy} onClick={() => run(async () => { await StudioCanvases.batchCancel(session.document.canvasId, batch.batchId); setIds([...ids]) })}>{uiText("请求取消")}</Button>}{batch.items?.some(item => item.generation?.actions?.retry) && <Button disabled={busy} onClick={() => run(async () => {
    const entered = await canvasPrompt(uiText("重试已释放费用的失败项：请输入本次允许预留的积分上限（不代表最终结算封顶）"))
  ...
  })}>{uiText("重试失败项")}</Button>}</Space>{batch.items?.map(item => <p key={`${item.clientItemId}:${item.outputIndex}`}>{item.generation?.nodeId}{item.generation?.shotId != null ? uiText(" / 镜头 {0}", item.generation.shotId) : ''} {uiText("· 输出") + " "}{item.outputIndex + 1} {uiText("· 任务") + " "}{item.generation?.generationId} · {item.generation?.billingState}</p>)}</section>)}{!batches.length && <p>{uiText("暂无本机记录的批次。其他设备的任务可在生成历史中查看。")}</p>}</div>
}
