...
export default function CanvasChatAttachment({ session, asset, file, queued = false, disabled = false, onAttach, onRemove }) {
  ...
  return <div className="canvas-v4-attachment" ref={card}>
    <button type="button" className="canvas-v4-attachment__preview" aria-label={`预览 ${name}`} onClick={() => { setOpen(true); void controller.current?.(!!error) }}>
      ...
      <span className="canvas-v4-attachment__info"><strong title={name}>{name}</strong><small>{queued ? '待关联' : ({ image: '图片', video: '视频', audio: '音频' }[kind] || '画布附件')}{asset?.sizeBytes != null && ` · ${(asset.sizeBytes / 1024 / 1024).toFixed(1)} MB`}<span><Eye size={11} />{error ? '点击重试预览' : '点击预览'}</span></small></span>
    </button>
    {onAttach && <Button type="text" size="small" className="canvas-v4-chat__attach-action" disabled={disabled} onClick={onAttach}>关联</Button>}
    {onRemove && <Button type="text" size="small" aria-label={`移除附件 ${name}`} disabled={disabled} icon={<X size={14} />} onClick={onRemove} />}
    <Modal title={name} open={open} onCancel={() => setOpen(false)} footer={null} width={820} destroyOnClose className="canvas-v4-attachment-modal">
      {open && <div className="canvas-v4-attachment__viewer">
        {busy && <Spin tip="正在加载附件…"><div style={{ height: 100 }} /></Spin>}
        {error && <Alert type="error" message={error} action={<Button size="small" disabled={busy} onClick={() => void controller.current?.(true)}>重试</Button>} />}
        ...
        {media && !['image', 'video', 'audio'].includes(kind) && <Alert type="info" message="此附件格式暂不支持预览" />}
      </div>}
    </Modal>
  </div>
}
