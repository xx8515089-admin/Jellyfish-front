...
export default function CanvasCloudChat({ theme = 'dark', session, enabled, active, save, assertReady, confirm, onClose, queuedFiles = [], removeQueuedFile }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      let delay = canvasPollDelay(...Object.values(details))
      try {
        ...
        if (stopped) return
        delay = Math.max(delay, canvasPollDelay(...tasks))
        setMessages(history)
        if (tasks.length) setDetails(previous => ({ ...previous, ...Object.fromEntries(tasks.map(task => [task.taskId, task])) }))
      } catch (reason) { delay = Math.max(delay, canvasPollDelay(reason)); if (!stopped) setError(canvasBillingError(reason)) }
  ...
  const createSession = async () => {
    ...
    if (!body) { body = { canvasId, title: uiText("新对话"), clientRequestId: canvasRequestId('chat-session') }; session.write('v4:chatSession', body) }
  ...
  const sessionMenu = [
    { key: 'refresh', icon: <RefreshCw size={14} />, label: uiText("刷新会话与模型"), disabled: busy },
    { key: 'showArchived', icon: showArchived ? <Check size={14} /> : <Archive size={14} />, label: showArchived ? uiText("隐藏已归档会话") : uiText("显示已归档会话"), disabled: busy },
    ...(current ? [{ type: 'divider' }, { key: 'archive', icon: current.archived ? <ArchiveRestore size={14} /> : <Archive size={14} />, label: current.archived ? uiText("取消归档当前会话") : uiText("归档当前会话"), disabled: busy || !!pending }] : []),
  ]
  const composerDisabled = busy || !!pending || current?.archived
  return <section className="canvas-v4-chat" data-theme={theme} aria-label={uiText("云端聊天")}>
    <header className="canvas-v4-chat__header">
      <div className="canvas-v4-chat__identity"><span className="canvas-v4-chat__brand"><MessageSquare size={17} /></span><div><strong>{uiText("云端聊天")}</strong><span>{uiText("灵感与对话，都留在画布里")}</span></div></div>
      <Tooltip title={uiText("关闭聊天")}><Button type="text" className="canvas-v4-chat__icon-button" aria-label={uiText("关闭聊天")} icon={<X size={17} />} onClick={onClose} /></Tooltip>
    </header>
    {enabled && <div className="canvas-v4-chat__sessionbar">
      <Select className="canvas-v4-chat__session-select" aria-label={uiText("云端聊天会话")} placeholder={uiText("新建或选择会话")} value={sessionId} disabled={busy || !!pending} suffixIcon={<ChevronDown size={13} />} options={sessions.filter(item => showArchived || !item.archived || String(item.sessionId) === String(sessionId)).map(item => ({ value: item.sessionId, label: uiText("{0}{1}", item.title, item.archived ? uiText("（已归档）") : '') }))} onChange={id => { selectedSession.current = id; setSessionId(id); setMessages([]); setDetails({}); void run(() => refresh(id)) }} />
      <Tooltip title={session.read('v4:chatSession') ? uiText("找回新会话") : uiText("新建对话")}><Button className="canvas-v4-chat__new" icon={<Plus size={15} />} disabled={busy || !ready || !!pending} onClick={() => run(createSession)}>{session.read('v4:chatSession') ? uiText("找回") : uiText("新对话")}</Button></Tooltip>
      ...
      } }}><Button type="text" className="canvas-v4-chat__icon-button" aria-label={uiText("会话选项")} icon={<MoreHorizontal size={18} />} /></Dropdown>
    </div>}
    <div className="canvas-v4-chat__notices">
      {error && <Alert type="error" showIcon message={error} closable onClose={() => setError('')} />}
      {!enabled && <Alert type="info" message={uiText("服务端尚未开放云端聊天")} />}
      {enabled && !ready && <Alert type="info" message={uiText("暂无可用聊天能力或模型，可在会话选项中刷新")} />}
      {enabled && pending && <Alert type="warning" message={uiText("上次提交结果尚未确认，请先找回原消息")} action={<Button disabled={busy} onClick={() => run(async () => { const task = await submitCanvasV4(session, 'execution', null, true); if (task?.sessionId != null) { selectedSession.current = task.sessionId; setSessionId(task.sessionId) }; setPrompt(''); setSelected([]); await refresh(task?.sessionId) })}>{uiText("找回提交")}</Button>} />}
    </div>
    {enabled && <>
      <div className="canvas-v4-chat__messages" aria-live="polite">
        {!messages.length && <div className="canvas-v4-chat__empty">
          ...
          <h3>{uiText("从一个想法开始")}</h3>
          <p>{uiText("聊聊故事、镜头或画面，")}<br />{uiText("也可以添加画布素材，一起完善创意。")}</p>
          ...
          <small>{uiText("消息与回复自动保存在当前画布")}</small>
        </div>}
        {messages.map(item => { const task = details[item.taskId]; return <article className="canvas-v4-message" key={item.taskId}>
          <section className="canvas-v4-message__user" aria-label={uiText("你的消息")}>
            <span className="canvas-v4-message__user-label">{uiText("你")}</span>
          ...
          {(task?.status ?? item.status) === 6 && <p className="canvas-v4-message__review-note">{uiText("已停止自动重发。核查原请求后再继续。")}</p>}
          <Space wrap className="canvas-v4-message__actions">{task?.actions?.cancel && <Button size="small" disabled={busy || task.cancelRequested} onClick={() => run(async () => { if (await confirm('取消消息', '调用后只能记录取消意愿，不能保证撤单或退款。')) { await StudioCanvases.executionCancel(canvasId, item.taskId); await refresh() } })}>{uiText("取消")}</Button>}{task?.actions?.retrySettlement && <Button size="small" disabled={busy} onClick={() => run(async () => { await StudioCanvases.executionRetrySettlement(canvasId, item.taskId); await refresh() })}>{uiText("重试结算")}</Button>}</Space>
        </article> })}
      </div>
      <div className="canvas-v4-chat__composer">
        {assetPickerOpen && <div className="canvas-v4-chat__asset-picker">
          <div className="canvas-v4-chat__picker-heading"><span>{uiText("画布素材")}</span><Button type="text" size="small" aria-label={uiText("收起素材选择")} icon={<X size={14} />} onClick={() => setAssetPickerOpen(false)} /></div>
          <Select mode="multiple" aria-label={uiText("聊天素材附件")} placeholder={uiText("选择素材，最多 8 份")} value={selected} disabled={composerDisabled} options={assets.filter(asset => !asset.removed && /^(image|video|audio)\//.test(asset.mimeType)).map(asset => ({ value: String(asset.assetId), label: asset.name || uiText("素材 {0}", asset.assetId) }))} onChange={setSelected} />
          {assetPage * 100 < assetTotal && <Button type="link" size="small" disabled={busy} onClick={() => run(() => loadAssets(assetPage + 1))}>{uiText("加载更多素材")}</Button>}
        </div>}
        <div className="canvas-v4-chat__input-card">
          ...
          <Input.TextArea className="canvas-v4-chat__input" aria-label={uiText("云端聊天消息")} placeholder={current?.archived ? uiText("此会话已归档，可在会话选项中恢复") : uiText("输入想法，或聊聊这张画布…")} value={prompt} maxLength={60000} autoSize={{ minRows: 3, maxRows: 6 }} disabled={composerDisabled} onChange={event => setPrompt(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && (event.ctrlKey || event.metaKey) && !event.nativeEvent.isComposing) { event.preventDefault(); send() } }} />
          <div className="canvas-v4-chat__model-row"><span>{uiText("模型")}</span><Select className="canvas-v4-chat__model-select" aria-label={uiText("云端聊天模型")} placeholder={uiText("选择可用模型")} value={modelId} disabled={composerDisabled} suffixIcon={<ChevronDown size={12} />} options={models.map(item => ({ value: item.modelId, disabled: item.available !== true, label: item.name + (item.available === true ? '' : `（${item.unavailableReason || '不可用'}）`) }))} onChange={setModelId} /></div>
          <div className="canvas-v4-chat__input-toolbar">
            <div className="canvas-v4-chat__attachment-tools">
              <Tooltip title={uiText("上传图片、音频或视频")}><Button type="text" className="canvas-v4-chat__tool" aria-label={uiText("上传聊天附件")} icon={<Paperclip size={16} />} disabled={composerDisabled || !ready} onClick={() => uploadRef.current?.click()}>{uiText("附件")}</Button></Tooltip>
              <Tooltip title={uiText("从当前画布选择素材")}><Button type="text" className="canvas-v4-chat__tool" aria-label={uiText("选择画布素材")} icon={<Images size={16} />} disabled={composerDisabled} onClick={() => { setAssetPickerOpen(value => !value); if (!assetPickerOpen) void run(() => loadAssets()) }}>{uiText("素材")}</Button></Tooltip>
            </div>
            <input ref={uploadRef} className="canvas-v4-chat__file-input" aria-label={uiText("选择上传文件")} type="file" accept="image/png,image/jpeg,video/*,audio/*" disabled={composerDisabled || !ready} onChange={uploadAttachment} />
            <Tooltip title={uiText("发送后按实际用量扣费")}><Button className="canvas-v4-chat__send" type="primary" aria-label={uiText("发送消息")} icon={<ArrowUp size={17} />} loading={busy} disabled={!ready || !!pending || !prompt.trim() || current?.archived || chatBlocked(messages)} onClick={send}>{uiText("发送")}</Button></Tooltip>
          </div>
        </div>
        <div className="canvas-v4-chat__composer-hint">{billingQuote?.unlimited === true && <span>{uiText("上次检查：不限额")}</span>}<span>{uiText("Ctrl / ⌘ + Enter 发送")}</span><Tooltip title={uiText("最多 8 份附件，合计 ≤50MiB。图片仅 PNG/JPEG，每份 ≤10MiB；音视频每份 ≤50MiB、≤300 秒。视频仅提供视觉内容，理解音轨请添加音频。")}><button type="button" className="canvas-v4-chat__limits" aria-label={uiText("查看附件限制")}><Info size={12} />{uiText("附件说明")}</button></Tooltip></div>
      </div>
    </>}
  </section>
}
