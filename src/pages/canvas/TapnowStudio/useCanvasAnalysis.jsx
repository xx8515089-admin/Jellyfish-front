...
export function CanvasAnalysisResult({ task, session }) {
  ...
  return <div className="canvas-analysis-results">
    {(result.warnings || []).map((warning, index) => <Alert key={index} type="info" message={warning === 'NO_AUDIO_TRACK' ? '没有检测到音轨，本次未调用模型。' : warning === 'NO_SPEECH' ? '音轨中没有识别到语音。' : String(warning)} />)}
    {result.hasAudio === false && !(result.warnings || []).includes('NO_AUDIO_TRACK') && <p>没有检测到音轨。</p>}
    {result.language && <Tag>语言：{result.language}</Tag>}
    ...
    {result.fullText && <details><summary>完整转写</summary><p className="canvas-analysis-prose">{result.fullText}</p></details>}
    {result.scenes?.map(scene => <section className="canvas-analysis-scene" key={scene.sceneId}>
      <header><strong>场景 {scene.sceneId}</strong><span>{scene.startSeconds}–{scene.endSeconds}s</span></header>
      <p>{scene.description}</p>
      {scene.cameraMovement && <p><b>镜头运动{scene.cameraMovementInferred ? '（推断）' : ''}：</b>{scene.cameraMovement}</p>}
      {scene.subjectDynamics && <p><b>主体动态：</b>{scene.subjectDynamics}</p>}{scene.atmosphere && <p><b>氛围：</b>{scene.atmosphere}</p>}
      <div className="canvas-analysis-frames">{scene.keyframes?.map(frame => <div key={frame.frameId}><CanvasChatAttachment session={session} asset={{ canvasId: task.canvasId, assetId: frame.assetId, mimeType: 'image/png', name: `${frame.timeSeconds}s · ${frame.role || '关键帧'}` }} /><small>{frame.timeSeconds}s · {frame.description}</small></div>)}</div>
      <p className="canvas-analysis-prose">{scene.prompts?.zh}</p><details><summary>英文提示词</summary><p className="canvas-analysis-prose">{scene.prompts?.en}</p></details>
...
export function useCanvasAnalysis({ session, snapshotRef, setNodes, save, assertReady, saveToUndoStack, confirm }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      for (const task of taskRef.current.filter(item => item.shouldPoll === true && item.status !== 6 && Date.now() >= (nextPoll.current.get(String(item.taskId)) || 0))) {
        ...
        catch (reason) { nextPoll.current.set(String(task.taskId), Date.now() + Math.max(2500, task.retryAfterMs || 0)); if (!stopped) setError(canvasBillingError(reason)) }
      }
      ...
      const delay = Math.max(1500, due.length ? Math.min(...due) : capabilities?.retryAfterMs || 2500)
  ...
  const taskCard = task => <article className="canvas-analysis-task" key={task.taskId}>
    <header><strong>{analysisOperations[task.operation] || task.operation}</strong><Tag>{executionStatus[task.status]}</Tag></header>
    <small>节点 {task.nodeId} · v{task.revisionNo} · {billingLabels[task.billingState] || task.billingState}{task.billingState === 'reserved' && <> · 预留 {credits(task.reservedCredits)}</>} · 实际 {credits(task.actualCredits)} 积分</small>
    {(task.error || task.errorCode) && <Alert type="warning" message={canvasBillingError(task)} />}
    {task.status === 6 && <Alert type="info" message="结果待核查，已停止自动重发。请查询原任务。" />}
    {task.status === 3 && <CanvasAnalysisResult task={task} session={session} />}
    {task.actions?.cancel && <small>取消后将停止后续处理；已经调用模型的费用仍按实际结算。</small>}
    <Space wrap><Button size="small" disabled={busy} onClick={() => run(async () => put(await detail(task.canvasId, task.taskId)))}>刷新状态</Button>
      {task.status === 3 && task.result && <Button type="primary" size="small" disabled={busy} onClick={() => apply(task)}>应用到节点</Button>}
      {task.actions?.cancel && <Button size="small" disabled={busy || task.cancelRequested} onClick={() => run(async () => put(await CanvasAnalysis.cancel(task.canvasId, task.taskId)))}>取消</Button>}
      {task.actions?.retry && task.status !== 6 && <Button size="small" disabled={busy} onClick={() => retry(task)}>按原输入重试</Button>}
      {task.actions?.retrySettlement && <Button size="small" disabled={busy} onClick={() => run(async () => put(await CanvasAnalysis.retrySettlement(task.canvasId, task.taskId)))}>重试结算</Button>}
    </Space>
  </article>
  const recovery = session?.read('analysis:submission') && <Button disabled={busy} onClick={() => run(async () => put(await submitAnalysis(session, null, true)))}>找回上次提交</Button>
  const panel = <div className="canvas-analysis-panel"><Space wrap><Button disabled={busy} onClick={() => run(async () => { await load(); await refresh() })}>刷新能力与任务</Button>{recovery}</Space>{error && <Alert type="warning" message={error} />}{!tasks.length && <Empty description="暂无分析任务" />}{tasks.map(taskCard)}<Pagination current={page} pageSize={20} total={total} onChange={setPage} /></div>
  ...
  const renderNode = node => {
    ...
    return <div className="canvas-analysis-node canvas-node__form pointer-events-auto nodrag nowheel" data-canvas-interactive onMouseDown={event => event.stopPropagation()} onPointerDown={event => event.stopPropagation()} onWheel={event => event.stopPropagation()}>
      <header><strong>云端媒体分析</strong><Button size="small" disabled={busy} onClick={() => run(load)}>刷新能力</Button></header>
      <Select aria-label="分析操作" value={operation} onChange={value => update({ analysisOperation: value, analysisModelId: undefined })} options={Object.entries(analysisOperations).map(([value, label]) => ({ value, label: `${label}${capabilities?.operationStatuses?.find(item => item.operation === value)?.available === false ? '（暂不可用）' : ''}` }))} />
      <Select aria-label="分析模型" placeholder="选择分析模型" value={validModel ? node.settings.analysisModelId : undefined} onChange={value => update({ analysisModelId: value })} options={models.map(model => ({ value: model.modelId, label: `${model.name}${model.supplierName ? ` · ${model.supplierName}` : ''}` }))} />
      {status?.available !== true && <Alert type="info" message={status?.reason || capabilities?.analysisUnavailableReason || '能力尚未加载，请刷新重试'} />}
      {catalogErrors[operation] && <Alert type="warning" message={catalogErrors[operation]} />}
      {status?.available === true && !models.length && !catalogErrors[operation] && <Alert type="info" message="当前操作没有可用分析模型，请刷新目录或联系管理员配置" />}
      {sourceError ? <Alert type="info" message={sourceError} /> : <div className="canvas-analysis-source"><small>来源：{source.title || source.id}</small>{source.content && (analysisSourceKind(source) === 'image' ? <img src={source.content} alt="分析来源" /> : analysisSourceKind(source) === 'audio' ? <audio controls src={source.content} /> : <video controls preload="metadata" src={source.content} />)}<small>已选关键帧 {source.type === 'input-image' ? 1 : source.selectedKeyframes?.length || 0} 个</small></div>}
      {operation === 'framePromptGenerate' ? <label>场景分组（秒）<InputNumber min={0.1} value={node.settings?.analysisSegmentSeconds ?? 4} onChange={value => update({ analysisSegmentSeconds: value })} /><small>使用来源节点已选关键帧；分组秒数不会自动抽帧。</small></label> : <Space wrap><label>开始秒<InputNumber min={0} placeholder="完整媒体" value={node.settings?.analysisStartSeconds} onChange={value => update({ analysisStartSeconds: value })} /></label><label>结束秒<InputNumber min={0} placeholder="完整媒体" value={node.settings?.analysisEndSeconds} onChange={value => update({ analysisEndSeconds: value })} /></label></Space>}
      <small>{operation === 'videoAnalyze' ? '分析画面、镜头与氛围；需要语音文本时请单独使用音轨转写。' : '完成后按实际用量结算，结果应用前可预览。'}</small>
      {quotes[node.id]?.unlimited === true && <small>上次检查：不限额，实际用量照常记录。</small>}
      {error && <Alert type="warning" message={error} />}{recovery}
      <Button type="primary" loading={busy} disabled={status?.available !== true || !validModel || !!sourceError || !!session?.read('analysis:submission') || tasks.some(task => task.nodeId === node.id && [1, 2, 6].includes(task.status))} onClick={() => execute(node.id, operation)}>开始{analysisOperations[operation]}</Button>
      {tasks.filter(task => task.nodeId === node.id).slice(0, 1).map(taskCard)}
      {!tasks.some(task => task.nodeId === node.id) && node.settings?.analysisResultData && <CanvasAnalysisResult session={session} task={{ canvasId: session.document.canvasId, result: node.settings.analysisResultData }} />}
    </div>
  }
  return { execute, renderNode, panel, capabilities, catalogs, error, apply, put, build: analysisEstimate, normalizeFrames: normalizeAnalysisFrames }
}
