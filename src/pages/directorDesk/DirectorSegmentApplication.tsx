...
export default function DirectorSegmentApplication({ segmentId }: { segmentId: string }) {
  ...
  return <>
    ...
    <Modal title="片段已应用的导演台图片" open={open} onCancel={() => setOpen(false)} footer={null} destroyOnClose>
      {application?.fileId != null ? <DirectorCaptureImageForm
        key={`${segmentId}-${application.applicationRevisionNo}`}
        segmentId={segmentId} aspectRatio={aspectRatio} includeCharacters={application.imageReferences.some((item) => item.referenceType === 1)}
