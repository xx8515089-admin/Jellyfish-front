...
export default function LazyDirectorVideo({ src, alt, width = 96, height = 72, mediaType = 'video', onRetry }: Props) {
  ...
  return <>
    <Button aria-label={uiText("播放{0}", alt || label)} style={{ width, height, flexShrink: 0, whiteSpace: 'normal' }} onClick={() => setOpen(true)}><PlayCircleOutlined />{uiText("播放{0}", label)}</Button>
    <Modal open={open} title={alt || uiText("参考{0}", label)} width={800} centered destroyOnClose footer={null} onCancel={() => setOpen(false)}>
      {error ? <Alert type="error" message={error} action={<Button onClick={async () => {
      ...
      }}>{uiText("重试")}</Button>} /> : open && resource?.source === src && resource ? (mediaType === 'video'
        ...
        : <div style={{ padding: 40, textAlign: 'center' }}><Spin /><p>{uiText("正在加载")}{label}…</p></div>}
    </Modal>
  </>
}
