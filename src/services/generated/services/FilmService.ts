...
import type { AtlasCallbackRequest } from '../models/AtlasCallbackRequest';
...
export class FilmService {
    ...
    /**
     * Atlas video generation callback
     * Accepts Atlas provider task status and coordinates local download/writeback idempotently.
     * @returns ApiResponse_dict_str__Any__ Successful Response
     * @throws ApiError
     */
    public static atlasTaskCallbackApiV1FilmTasksAtlasCallbackPost({
        requestBody,
    }: {
        requestBody: AtlasCallbackRequest,
    }): CancelablePromise<ApiResponse_dict_str__Any__> {
        return __request(OpenAPI, {
            method: 'POST',
            url: '/api/v1/film/tasks/atlas/callback',
            body: requestBody,
            mediaType: 'application/json',
            errors: {
                422: `Validation Error`,
    ...
    /**
     * 轮询并恢复已有 Atlas provider task
     * 只 poll 已存在的 provider_task_id；不会提交新的 Atlas 生成任务。
     * @returns ApiResponse_dict_str__Any__ Successful Response
     * @throws ApiError
     */
    public static recoverAtlasTaskApiV1FilmTasksTaskIdRecoverAtlasPost({
        taskId,
        dryRun = false,
    }: {
        taskId: string,
        /**
         * 仅检查 provider 状态，不落库下载
         */
        dryRun?: boolean,
    }): CancelablePromise<ApiResponse_dict_str__Any__> {
        return __request(OpenAPI, {
            method: 'POST',
            url: '/api/v1/film/tasks/{task_id}/recover-atlas',
            path: {
                'task_id': taskId,
            },
            query: {
