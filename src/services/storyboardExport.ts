...
export type ExportRequest = { scriptImportId: number; dimension: 'episode' | 'segment'; episodeId?: number; scope: 'all' | 'custom'; start?: number; end?: number; content: 'media' | 'audio' | 'mixed'; mediaSelections?: { segmentId: number; generationId: number; mediaType: 'image' | 'video' }[] }
export type ExportProblem = { code: string; message: string; episodeIndex?: number; segmentIndex?: number }
export type ExportPreview = { canExport: boolean; message: string; fileName: string; imageCount: number; videoCount: number; audioCount: number; totalBytes: number; problems: ExportProblem[]; items: { segmentId: number; generationId: number; mediaType: 'image' | 'video' | 'audio' }[] }
export class StoryboardExportError extends Error {
  constructor(message: string, public problems: ExportProblem[] = []) { super(message) }
}
...
export function freezeExportRequest(request: ExportRequest, preview: ExportPreview): ExportRequest {
  const { mediaSelections: _selections, ...rest } = request
  if (request.content === 'audio') return rest
  return { ...rest, mediaSelections: preview.items.filter((item) => item.mediaType === 'image' || item.mediaType === 'video').map((item) => ({ segmentId: exportInteger(item.segmentId), generationId: exportInteger(item.generationId), mediaType: item.mediaType as 'image' | 'video' })) }
}
...
async function post(path: string, request: ExportRequest, signal: AbortSignal) {
  ...
  if (!response.ok || response.headers.get('Content-Type')?.includes('json')) {
    ...
    if (!response.ok || (body?.code ?? 200) >= 400 || path === 'zip') throw new StoryboardExportError(body?.message || '导出失败，请重试', body?.data?.problems || [])
...
export async function downloadStoryboardZip(request: ExportRequest, signal: AbortSignal, onPreview: (preview: ExportPreview) => void) {
  const preview = await post('preview', request, signal) as ExportPreview
  onPreview(preview)
  ...
  const response = await post('zip', freezeExportRequest(request, preview), signal) as Response
